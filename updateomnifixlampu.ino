...
void stopAllMotorsBase() {
  ...
  // KOMENTAR/HAPUS BARIS DI BAWAH INI:
...
void jalankanKalibrasi() {
  ...
  // ------------------------------------------------------------------
  // STEP 0: Inisialisasi motor baru - langsung full PWM dengan kick
  // ------------------------------------------------------------------
  if (calibStep == 0) {
    ...
    // === FIX: Kick-start - off sebentar lalu full power ===
    // Tujuan: lepaskan rotor dari dead-spot / posisi stall awal
  ...
  // ------------------------------------------------------------------
  // STEP 1: Tunggu 3 detik agar motor mencapai top speed
  // ------------------------------------------------------------------
  else if (calibStep == 1) {
  ...
  // ------------------------------------------------------------------
  // STEP 2: Baca top speed, hitung prediksi PWM awal
  // ------------------------------------------------------------------
  else if (calibStep == 2) {
    int maxRPM = getRPM(pinSekarang);

    // === FIX: Handling stall - retry dengan kick ulang ===
    if (maxRPM == 0) {
      stallRetryCount++;
      ...
      if (stallRetryCount <= MAX_STALL_RETRY) {
        // Kick ulang: matikan sebentar lalu full power lagi
        // delay 300ms lebih lama agar rotor settle ke posisi berbeda
        motorWrite(calibMotor, 0);
        delay(300);
        motorWrite(calibMotor, 255);
        currentPWM[calibMotor] = 255;
        calibTimer = millis();
        calibStep = 1; // ulangi tunggu 3 detik
      } else {
        // Gagal total setelah MAX_STALL_RETRY kali, skip motor ini
        ...
        // Cek apakah semua motor sudah selesai (karena skip)
        if (calibMotor > 4) {
    ...
    // Motor berhasil berputar, lanjut proses normal
    if (maxRPM > 1000) return;
    stallRetryCount = 0; // reset counter karena berhasil
  ...
  // ------------------------------------------------------------------
  // STEP 3: Tunggu 2 detik setelah ganti PWM ke prediksi
  // ------------------------------------------------------------------
  else if (calibStep == 3) {
  ...
  // ------------------------------------------------------------------
  // STEP 4: Baca RPM aktual, koreksi PWM jika perlu
  // ------------------------------------------------------------------
  else if (calibStep == 4) {
  ...
  // ------------------------------------------------------------------
  // STEP 5: Uji stabilitas 10 detik
  // ------------------------------------------------------------------
  else if (calibStep == 5) {
    int aktualRPM = getRPM(pinSekarang);
    if (aktualRPM == 0 || aktualRPM > 1000) return;
