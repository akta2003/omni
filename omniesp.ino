...
// NOTE: Untuk Arduino-ESP32 v3.x, cukup Arduino.h.
// #include "driver/ledc.h"  // <- tidak diperlukan untuk API Arduino LEDC baru
...
static constexpr uint16_t REG_PULSEMS  = 10;  // <-- BARU: durasi auto-off untuk belok/diagonal (ms)
...
uint16_t lastPulseMs = 300; // <-- cache pulse
...
void motorWrite(uint8_t idx, int speed) {
  ...
  // Arduino-ESP32 v3.x: jika pakai "channel", gunakan ledcWriteChannel()
...
void STOP_ALL() {
  cancelPulse(); // <-- penting: stop mematikan pulse
...
void MAJU() {
  cancelPulse(); // <-- supaya pulse lama tidak nyangkut
...
void KANAN() {
  ...
  startPulse(HREG[REG_PULSEMS]);   // <-- auto-off
}
...
// diagonal (setengah roda jalan) -> juga auto-off
void KANAN_ATAS() {
...
void reApplyMotion() {
  // saat config berubah, kita re-apply sesuai mode sekarang
  // NOTE: kalau mode sekarang belok/diagonal, ini akan startPulse lagi (memperpanjang durasi).
  switch (currentMode) {
...
// =======================
// Apply register changes (PWM/scale/stopmode/pulsems)
// =======================
void syncConfigFromRegisters() {
  ...
  // pulse ms (durasi auto-off belok)
  ...
  if (HREG[REG_PULSEMS] != lastPulseMs) {
    ...
    // tidak wajib changedMotion, tapi kalau mau langsung "kerasa" saat belok, boleh reapply:
    // changedMotion = true;
  }
...
void setup() {
  ...
  // =======================
  // LEDC init (API baru Arduino-ESP32 v3.x)
  // =======================
  // Kita tetap pakai "channel" 0..7 seperti desain awal.
  // ledcAttachChannel(pin, freq, resolution_bits, channel);
  // Return bool (true kalau sukses). Di sini kita abaikan, tapi bisa dicek kalau mau.
  ...
  HREG[REG_PULSEMS]  = 300;   // <-- BARU: default belok 300ms
  lastPulseMs        = HREG[REG_PULSEMS];
  ...
  Serial.printf("SLAVE_ID=%d | PWM: %d %d %d %d | SCALE=%d | PULSEMS=%d\n",
                SLAVE_ID, p1, p2, p3, p4, HREG[REG_SCALE], HREG[REG_PULSEMS]);
}
...
void loop() {
  ...
  // AUTO-OFF pulse untuk belok/diagonal
  if (pulseActive) {
    if ((int32_t)(millis() - pulseUntilMs) >= 0) {
      pulseActive = false;
      HREG[REG_CMD] = CMD_STOP; // biar master tahu sudah stop
      STOP_ALL();
      lastCmd = CMD_STOP;       // cegah STOP diproses ulang di switch bawah
    }
  }
