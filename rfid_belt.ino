...
void setup() {
  ...
  HREG[REG_MOTOR_CMD] = 0; // Default Maju
...
void loop() {
  ...
  if (mode == 1) { 
    ...
    // PERBAIKAN: Tambahkan syarat HREG[REG_MOTOR_CMD] == 1
    // Artinya: Hanya bereaksi jika motor memang sedang diperintahkan jalan
    if (prox == 1 && lastProx == 0 && !mode1Active && HREG[REG_MOTOR_CMD] == 1) {
        HREG[REG_MOTOR_CMD] = 0; // Stop sementara
        timerMode1 = millis();
        mode1Active = true;
        Serial.println("Box Terdeteksi! Stop sementara 3 detik...");
    }
    
    // Resume motor setelah 3 detik
    if (mode1Active && (millis() - timerMode1 >= 3000)) {
        HREG[REG_MOTOR_CMD] = 1; // Kembali jalan
        mode1Active = false;
