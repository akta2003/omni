...
// COMMAND AUTO-STOP (Proximity + Delay 3s): 20 (Maju), 21 (Kanan), 22 (Kiri)
...
// Faktor Koreksi (Tachometer / Encoder)
// Nilai di bawah ini khusus untuk SLAVE_ID 2
// Jika ganti ke Slave 2, ubah jadi: {0.0, 1.45, 1.47, 1.39, 1.44}
// Jika ganti ke Slave 5, ubah jadi: {0.0, 1.27, 1.36, 1.37, 1.30}
// Jika ganti ke Slave 8, ubah jadi: {0.0, 1.30, 1.27, 1.27, 1.29}
...
// Register yang tidak terpakai dihapus, tetapi urutan index dipertahankan
...
void executeCommand(uint16_t cmd) {
  ...
  switch (cmd) {
    ...
    case CMD_KANAN_AUTO:  setArah(1,-1,1,-1,  false,true,false,false); systemState = RUNNING_AUTO; autoStopActive = false; break;
    case CMD_KIRI_AUTO:   setArah(-1,1,-1,1,  false,false,true,false); systemState = RUNNING_AUTO; autoStopActive = false; break;
...
int getRPM(int pinEncoder, int motorIndex) {
  ...
  // RPM mentah bawaan rumus awal
  int rpmMentah = 30000000.0 / pulsa;
  
  // Kompensasi agar sesuai Tachometer
  int rpmKoreksi = rpmMentah * faktorKoreksi[motorIndex];
  
  return rpmKoreksi;
}

void jalankanKalibrasi() {
  int pinSekarang = (calibMotor == 1) ? encoderPin1 :
                    (calibMotor == 2) ? encoderPin2 :
                    (calibMotor == 3) ? encoderPin3 : encoderPin4;

  if (calibStep == 0) {
    for (int i = 1; i <= 4; i++) memoriRPM[i] = 0;
    stallRetryCount = 0;
    testPWM = 255;
    currentPWM[calibMotor] = testPWM;

    motorWrite(calibMotor, 0);
    delay(150);
    motorWrite(calibMotor, 255);

    Serial.printf("\n-> [M%d] Kick-Start! Cek Top Speed. Tunggu 3 detik...\n", calibMotor);
    calibTimer = millis();
    calibStep = 1;
  }
  else if (calibStep == 1) {
    ...
    if (maxRPM == 0) {
      stallRetryCount++;
      Serial.printf("   [M%d] Motor STALL! Retry %d/%d - Re-Kick...\n",
                     calibMotor, stallRetryCount, MAX_STALL_RETRY);

      if (stallRetryCount <= MAX_STALL_RETRY) {
        motorWrite(calibMotor, 0);
        delay(300);
        motorWrite(calibMotor, 255);
        currentPWM[calibMotor] = 255;
        calibTimer = millis();
        calibStep = 1;
      } else {
        Serial.printf("   [M%d] GAGAL START setelah %d kali retry! Skip motor.\n",
                       calibMotor, MAX_STALL_RETRY);
        stallRetryCount = 0;
        motorWrite(calibMotor, 0);
        calibMotor++;
        calibStep = 0;
        ...
        if (calibMotor > 4) {
          Serial.println("\n=== KALIBRASI SELESAI (ada motor yang di-skip) ===");
          prefs.begin("pwm", false);
          prefs.putUChar("pwm1", basePWM[1]); prefs.putUChar("pwm2", basePWM[2]);
          prefs.putUChar("pwm3", basePWM[3]); prefs.putUChar("pwm4", basePWM[4]);
          prefs.end();
    ...
    if (maxRPM > 1000) return;
    stallRetryCount = 0;

    memoriRPM[calibMotor] = maxRPM;
    ...
    int aktualRPM = getRPM(pinSekarang, calibMotor);
    if (aktualRPM == 0 || aktualRPM > 1000) return;

    memoriRPM[calibMotor] = aktualRPM;
...
void jalankanOperasi() {
  for (int i = 1; i <= 4; i++) {
    ...
    motorWrite(i, basePWM[i] * targetDir[i]);
  }
}
...
void setup() {
  ...
  basePWM[1] = prefs.getUChar("pwm1", 200);
  basePWM[2] = prefs.getUChar("pwm2", 200);
  basePWM[3] = prefs.getUChar("pwm3", 200);
  basePWM[4] = prefs.getUChar("pwm4", 200);
  prefs.end();
...
void loop() {
  ...
  if (systemState == CALIBRATING) { 
    ...
    jalankanOperasi(); // Motor tetap berputar sesuai targetDir

    // Trigger timer jika proximity aktif dan timer belum berjalan
    if (HREG[REG_PROX] == 1 && !autoStopActive) {
      autoStopActive = true;
      autoStopTimer = millis();
      Serial.println("Proximity mendeteksi objek! Timer 3 detik dimulai...");
    }

    // Jika timer aktif dan sudah mencapai 3 detik
    if (autoStopActive && (millis() - autoStopTimer >= 3000)) {
      
      // --- FIX: Panggil executeCommand agar mematikan motor sekaligus LED ---
      executeCommand(CMD_STOP_COAST); 
      
      // Sinkronisasi kembali register agar Master/PLC tahu posisi sudah stop
      HREG[REG_CMD] = CMD_STOP_COAST; 
      lastCmd = CMD_STOP_COAST;
      autoStopActive = false;
      
      Serial.println("Auto-Stop dieksekusi!");
    }
  }
  else { 
    stopAllMotorsBase(); 
